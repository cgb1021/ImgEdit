...
function save(img, state, method) {
  ...
  var dWidth = canvas.width = width * state.scale;
  var dHeight = canvas.height = height * state.scale;
  if (state.angle) {
    if (state.angle !== 1) {
  ...
  (0, _createClass3.default)(ImgEdit, [{
    ...
    value: function toBlob() {
      ...
      return new _promise2.default(function (resolve, reject) {
        if (!_this3.img) {
          reject(new Error(_this3.img));
        } else {
          save(_this3.img, data[_this3.id], 'toBlob', function (res) {
            resolve(res);
          }, mime, quality);
        }
      });
    ...
    value: function reset(noDraw) {
      ...
      this.viewScale = Math.min(1, this.canvas.width / state.sprite.width, this.canvas.height / state.sprite.height); // 在画布上的显示比例（scale）
    ...
    value: function scale(s, wheel) {
      ...
      var _getSize3 = getSize(state),
          width = _getSize3.width,
          height = _getSize3.height;
