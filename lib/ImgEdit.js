...
var ImgEdit = function () {
  ...
  (0, _createClass3.default)(ImgEdit, [{
    ...
    value: function scale(_scale) {
      ...
      if (!state.img) return this;
      // 放大比例不能小于1或大于10
      var viewScale = state.viewScale + _scale;
      if (viewScale < .1 || viewScale > 10) {
        return this;
      } else {
        state.viewScale = viewScale;
      }
      if ((state.offsetX || state.offsetY) && state.offsetX - state.cx > 0 && state.offsetY - state.cy > 0 && state.offsetX < state.cx + state.width * state.viewScale && state.offsetY < state.cy + state.height * state.viewScale) {
        // 在图片范围内，以鼠标位置为中心
